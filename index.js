...
const userStates = {};
const referralMap = {};
const lastAdminMessage = {}; // Track last admin message to prevent duplicates

// Debounce utility to prevent duplicate messages
  ...
  try {
    for (const price of defaultPrices) {
      ...
      if (!existingPrice) {
        ...
        console.log(`Initialized price: ${price.type} = ${price.value}`);
      }
    }
  } catch (error) {
    console.error("Error initializing prices:", error.message);
  }
}

initializePrices();

// Optimized sendAdminPanel with debounce
const sendAdminPanel = debounce(async (chatId) => {
  const now = Date.now();
  if (lastAdminMessage[chatId] && now - lastAdminMessage[chatId] < 1000) {
    console.log(`Skipped duplicate admin panel message for chatId: ${chatId}`);
    return;
  }

  lastAdminMessage[chatId] = now;
  console.log(`Sending admin panel to chatId: ${chatId}`);
...
bot.onText(/\/start(?:\?ref=(\d+))?/, async (msg, match) => {
  ...
  if (userSteps[chatId] || userStates[chatId]) {
    console.log(`Skipping duplicate /start for chatId: ${chatId}`);
    return;
  }
  ...
  try {
    ...
    if (existingUser) {
      console.log(`User already exists: telegramId: ${userId}`);
      delete userSteps[chatId];
      delete userStates[chatId];
    ...
    console.error(`Error processing /start for userId: ${userId}:`, error.message);
...
bot.on("contact", async (msg) => {
  ...
  if (userSteps[chatId]?.step !== "waiting_for_phone") {
    console.log(`Invalid step for chatId: ${chatId}, expected 'waiting_for_phone'`);
    return;
  }
  ...
  if (!phone.startsWith("+998")) {
    if (phone.startsWith("998")) {
      ...
      console.log(`Invalid phone number for userId: ${userId}, phone: ${phone}`);
  ...
  try {
    ...
    try {
      ...
      console.log(`New user created: telegramId: ${userId}, phone: ${phone}`);

      // Handle referral
      if (referralMap[userId]) {
        const refUser = await User.findOne({ telegramId: referralMap[userId] }).session(session);
        if (refUser) {
          ...
          console.log(`Referral bonus added for referrerId: ${referralMap[userId]}`);
        } else {
          console.log(`Referrer not found for referrerId: ${referralMap[userId]}`);
        }
      }
    ...
    console.error(`Error creating user for telegramId: ${userId}:`, error.message);
    await bot.sendMessage(
      chatId,
...
bot.on("callback_query", async (query) => {
  ...
  if (data === "back_to_main") {
    delete userSteps[chatId];
    delete userStates[chatId];
    return userId === ADMIN_CHAT_ID ? sendAdminPanel(chatId) : sendMainMenu(chatId);
  }
});
...
bot.on("message", async (msg) => {
  ...
  if (userId === ADMIN_CHAT_ID) {
    ...
    if (userStates[chatId]?.step === "adjust_stars") {
      ...
      try {
        ...
        console.error("Error adjusting stars:", error.message);
        return bot.sendMessage(chatId, "❌ Xatolik yuz berdi.", {
          reply_markup: { keyboard: [["🔙 Ortga"]], resize_keyboard: true },
        });
      }
    ...
    if (text === "💵 Narxlarni o‘zgartirish") {
      try {
        ...
        return bot.sendMessage(chatId, "❌ Narxlarni olishda xatolik.", {
          reply_markup: { keyboard: [["🔙 Ortga"]], resize_keyboard: true },
        });
      }
    }
    ...
    if (userStates[chatId]?.step === "update_price") {
      ...
      try {
        ...
        return bot.sendMessage(chatId, "❌ Narxni o‘zgartirishda xatolik.", {
          reply_markup: { keyboard: [["🔙 Ortga"]], resize_keyboard: true },
        });
      }
    }
  ...
  if (userStates[chatId]?.step === "waiting_for_star_amount") {
    ...
    try {
      ...
      console.error("Error calculating star price:", error.message);
      return bot.sendMessage(chatId, "❌ Xatolik yuz berdi.", {
        reply_markup: { keyboard: [["🔙 Ortga"]], resize_keyboard: true },
      });
    }
  ...
  if (userStates[chatId]?.step === "waiting_for_star_recipient") {
    ...
    try {
      ...
      console.error("Error sending star order:", error.message);
      return bot.sendMessage(chatId, "❌ Buyurtma yuborishda xatolik.", {
        reply_markup: { keyboard: [["🔙 Ortga"]], resize_keyboard: true },
      });
    }
  }
  ...
  if (text === "⭐ Premium sotib olish") {
    ...
    try {
      ...
      console.error("Error displaying premium prices:", error.message);
      return bot.sendMessage(chatId, "❌ Narxlarni olishda xatolik.", {
        reply_markup: { keyboard: [["🔙 Ortga"]], resize_keyboard: true },
      });
    }
  ...
  if (userStates[chatId]?.step === "choosing_package" && text.startsWith("📦")) {
    ...
    try {
      ...
      console.error("Error processing premium package:", error.message);
      return bot.sendMessage(chatId, "❌ Xatolik yuz berdi.", {
        reply_markup: { keyboard: [["🔙 Ortga"]], resize_keyboard: true },
      });
    }
  ...
  if (userStates[chatId]?.step === "choosing_recipient") {
    ...
    try {
      ...
      console.error("Error sending premium order:", error.message);
      return bot.sendMessage(chatId, "❌ Buyurtma yuborishda xatolik.", {
        reply_markup: { keyboard: [["🔙 Ortga"]], resize_keyboard: true },
      });
    }
